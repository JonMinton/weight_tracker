...
delta_data %>% 
  ...
  geom_text(x = as.Date("2020-08-14"), y = 77.5, label = lm_eqn(mod), colour = "black", hjust = 0)

ggsave("weight_chart.png", height = 20, width = 25, units = "cm", dpi = 300)
...
delta_data %>% 
  ...
  group_by(week) %>% 
  nest() %>% 
  mutate(mdl = map(.x = data, lm, formula = min_max_weight ~ day_alone + tod, data = .))
